...
6. Register the exact downloaded, extracted, or user-provided input and run data-analysis QC before Seurat/Scanpy execution.
7. Review cell annotation evidence and generate a downstream proposal before CellChat or Monocle3.
8. Render or run CellChat/Monocle3 only after the user explicitly approves the proposed cell groups, microenvironment, lineage, and root choice.
9. The user runs approved code locally, in RStudio, Python, Colab, or an institutional server when tools are available.
10. Skill checks data synchronization, result quality, known-bad rerun artifacts, logs, and writes cautious interpretation, Methods, Results, legends, limitations, and validation suggestions.
...
7. **Data-analysis QC**: before running Seurat/Scanpy, use `references/data_analysis_qc_rules.md`; for MCP, call `register_input_dataset` for manual inputs or rely on `start_geo_download` / `start_extract_workspace_archive` to create `data_input_manifest.json`, then call `validate_data_analysis_qc`. Without MCP, run `scripts/validate_data_sync.py`.
8. **Annotation and result review**: use `agents/05_result_quality_checker.md` and optionally run `scripts/validate_result_bundle.py`.
9. **Downstream proposal gate**: before CellChat or Monocle3, run `scripts/propose_downstream_modules.py`, present `downstream_proposal.md`, and wait for user approval.
10. **Approved downstream execution**: only after approval, render `scripts/course_adapted/03_cellchat_from_seurat.R` or `scripts/course_adapted/04_monocle3_from_seurat.R` with the approved scope.
11. **Biological interpretation/report**: use agents 06-08, then run `scripts/write_analysis_report.py` to generate a durable report from the actual result tables.
...
python scripts/validate_data_sync.py --result-dir analysis/run1 --input-path /path/to/data --input-type 10x_mtx --manifest analysis/run1/data_input_manifest.json --out-md analysis/run1/data_analysis_qc.md
...
2. Data-analysis QC: confirm that the actual `input_path` for analysis matches the planned/downloaded/extracted/registered dataset; write `data_input_manifest.json` and `data_analysis_qc.md` before running Seurat/Scanpy.
3. Single-cell QC and filtering: `10_quality_control.R` -> `references/singlecell_qc_rules.md` and `01_seurat_v5_core_pipeline.R`.
4. First normalization, variable features, scaling, PCA, and optional Harmony: `11_normalization_decontx_harmony.R` -> `01_seurat_v5_core_pipeline.R` or `00_multi_sample_merge_harmony.R`.
5. Doublet detection: `12_doublet_finder.R` or `13_scdblfinder.R`; propose it when raw counts and per-sample/loading-batch metadata support it, otherwise explain why it is skipped.
6. Doublet removal: remove predicted doublets per sample/loading batch, without hard-coding DoubletFinder classification column names.
7. Post-doublet re-normalization and reprocessing: `14_post_doublet_normalization.R`; rerun normalization, variable features, scaling, PCA, neighbors, clustering, and UMAP on retained cells.
8. Clustering and resolution review: `15_clustering_resolution.R` -> `01_seurat_v5_core_pipeline.R`; review cluster stability and marker support.
9. Marker detection: `23_marker_detection_methods.R` -> `01_seurat_v5_core_pipeline.R` and `02_marker_enrichment_from_seurat.R`.
10. Cell annotation after clustering and marker review: `16_manual_cell_annotation.R`, `17_singler_annotation.R`, `18_scina_annotation.R`, `21_transferdata_annotation.R`, `22_scpred_annotation.R` -> `05_singler_cell_annotation.R` plus manual marker evidence.
11. Downstream proposal gate: only after annotation evidence is reviewed, propose CellChat, Monocle3/pseudotime, CNV, enrichment, or deconvolution scope and wait for user approval before rendering or running those modules.
...
- Data-analysis QC showing that the analyzed dataset matches the planned/downloaded/extracted/registered input.
...
- `data_input_manifest.json` and `data_analysis_qc.md` when running through MCP or a local reproducible workflow.
