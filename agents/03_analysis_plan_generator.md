...
C. 第一次标准化、降维和批次处理
D. 双细胞检测与去除方案
E. 去除双细胞后的重新标准化、重新降维和重新聚类
F. 分群聚类、分辨率检查和 marker 检测
G. 细胞注释和注释证据复核
H. 组间比较
I. 功能富集
J. 下游解释性模块候选方案
...
5. 去除双细胞：按 sample/loading batch 移除 predicted doublets，不能硬编码分类列名。
6. 去除双细胞后重新标准化：`14_post_doublet_normalization.R`；重新跑 NormalizeData、FindVariableFeatures、ScaleData、PCA、neighbors、clusters、UMAP。
7. 分群聚类和分辨率检查：`15_clustering_resolution.R` -> `01_seurat_v5_core_pipeline.R`。
8. Marker 检测：`23_marker_detection_methods.R` -> `01_seurat_v5_core_pipeline.R` 和 `02_marker_enrichment_from_seurat.R`。
9. 细胞注释：`16_manual_cell_annotation.R`、`17_singler_annotation.R`、`18_scina_annotation.R`、`21_transferdata_annotation.R`、`22_scpred_annotation.R` -> `05_singler_cell_annotation.R` 加人工 marker 复核。
10. 下游模块：只有在注释证据复核后，才能提出 CellChat、拟时序、CNV、反卷积等方案，并等待使用者确认。
